...
class FatSecretAPI{
	...
	*/
	...
	function foodget($id, $auth, &$sessionKey){
		$url = FatSecretAPI::$base . 'format=json&food_id='.$id.'&method=food.get';
		...
		return json_decode($this->GetQueryResponse($normalizedUrl, $normalizedRequestParameters . '&' . OAuthBase::$OAUTH_SIGNATURE . '=' . urlencode($signature)), 1);
	}

	function foodsearch($term, $auth, &$sessionKey){
		$url = FatSecretAPI::$base . 'format=json&max_results=50&search_expression='.$term.'&method=foods.search';
